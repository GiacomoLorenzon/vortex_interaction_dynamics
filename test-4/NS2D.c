...
#define MAXLEVEL 9
double Reynolds = 10000.;    // Reynolds based on Gamma1
double D0 = 2.;             // Distance btw vortices  (reference)
double dom_size = 10;        // Domain size
double Gamma1 = 1;          // Vortex 1 : circulation (reference)
double a1 = 0.2;            // Vortex 1 : core size
double Gamma2 = 0.5;          // Vortex 2 : circulation
double a2 = 0.2;            // Vortex 2 : core size
double Gamma3 = -1/3;          // Vortex 3 : circulation (reference)
double a3 = 0.2;            // Vortex 3 : core size
// double Gamma4 = -1;          // Vortex 4 : circulation
// double a4= 0.2;            // Vortex 4 : core size
double t_final = 100;        // Final simulation time
...
event init (i = 0)
{
 foreach() 
    // Case 1:
    // omega[] = Gamma1/(pi*sq(a1))*exp(-(sq(x) + sq(y))/sq(a1));
    
    // Case 2a:
     omega[] =
         Gamma1/(pi*sq(a1))*exp(-(sq(x) + sq(y))/sq(a1))
       + Gamma2/(pi*sq(a2))*exp(-(sq(x - 1) + sq(y))/sq(a2))
       + Gamma2/(pi*sq(a2))*exp(-(sq(x - 1) + sq(y - 1/sqrt(3)))/sq(a2));
}
...
event logfile(t += 0.2; t <= t_final)
{
  ...
  { //    Case with nonzero circulation
    ...
    r = xcagafuori * xcagafuori + ycagafuori * ycagafuori;
    r = sqrt(r);
    ...
    fprintf(stderr,
            ...
            sumx, sumy, sum2, circ);
    if (t == 0)
      fprintf(fp, "time x_c y_c a circ r\n");
    fprintf(fp, "%g %g %g %g %g %g\n",
            t, sumx, sumy, sum2, circ, r);
  }
  else
  { //    Case with zero circulation
    fprintf(stderr, "  circ=%12.5lf\n", circ);
    if (t == 0)
      fprintf(fp, "time circ\n");
    fprintf(fp, "%g %g\n", t, circ);
  }
}
