...
double t_final = 70;        // Final simulation time
...
int main() {
  ...
  omega_min = -omega_max;            // set to 0 (monopoles) or to -omega_max (dipole)
  L0 = dom_size;
  origin (-L0/2., -L0/2.);
  size(L0);
  init_grid (1 << MAXLEVEL);
  // TOLERANCE = 1e-5;  // originary TOLERANCE = 1e-7;
  run();
}
...
{
 foreach()
    omega[] = Gamma1/(pi*sq(a1))*exp(-(sq(x + D0/2) + sq(y+5))/sq(a1))
     + Gamma2/(pi*sq(a2))*exp(-(sq(x - D0/2) + sq(y+5))/sq(a2));
}
...
//       sumy += dv()*y*omega[];
//   }
//   static FILE * fp = fopen ("data3.out", "w");
//   fprintf (stderr, "> t=%10.5lf    i=%d    dt=%10.5lf\n", t, i, dt);
//
//   if (circ>1e-4 || circ<-1e-4) {       //    Case with nonzero circulation
...
event logfile(t += 0.2; t <= t_final)
{
  ...
  if (circ > 1e-4 || circ < -1e-4)
  { //    Case with nonzero circulation
    sumx = sumx / circ;
    sumy = sumy / circ;
    foreach (reduction(+
                       : sum2))
    ...
    fprintf(stderr,
            ...
            sumx, sumy, sum2, circ);
    if (t == 0)
      fprintf(fp, "time x_c y_c a circ\n");
    fprintf(fp, "%g %g %g %g %g\n",
            t, sumx, sumy, sum2, circ);
  }
  else
  { //    Case with zero circulation

    dist = (xmax-xmin) * (xmax-xmin) + (ymax-ymin) * (ymax-ymin);
    dist = sqrt(dist);
    
    double vel = (ymax - ymax_old)/0.2;

    fprintf(stderr, "  circ=%12.5lf\n", circ);
    if (t == 0)
      fprintf(fp, "time circ dist vel\n");
    fprintf(fp, "%g %g %g %g\n", t, circ, dist, vel);
  }
}
...
event movie (t += 0.2) {
  ...
  vorticity (u, omega);
  // Vorticity
  output_ppm (omega, linear = true, file = "vort3.mp4", map = cool_warm,
              min = omega_min, max = omega_max, n = 512);

  // Discretization level
  foreach()
    omega[] = level;
  output_ppm (omega, spread = 2, file = "level3.mp4", n=512);

  // Velocity
  foreach()
    omega[] = sqrt(sq(u.x[]) + sq(u.y[]));
  output_ppm (omega, linear = true, file = "velo3.mp4",
              map = gray, n = 512);
}
