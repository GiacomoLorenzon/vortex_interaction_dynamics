...
event init (i = 0)
{
 foreach() 
    ...
    // Case 2a:
     omega[] =
         Gamma1/(pi*sq(a1))*exp(-(sq(x + D0/2) + sq(y))/sq(a1))
       + Gamma2/(pi*sq(a2))*exp(-(sq(x + D0/4) + sq(y))/sq(a2))
       + Gamma3/(pi*sq(a2))*exp(-(sq(x - D0/4) + sq(y))/sq(a2))
       + Gamma4/(pi*sq(a2))*exp(-(sq(x - D0/2) + sq(y))/sq(a2));
}
