...
#define MAXLEVEL 9
double Reynolds = 100000.;     // Reynolds based on Gamma1
...
int main()
{
  mu = rho * Gamma1 / Reynolds; // viscosita dinamica
...
{
  foreach ()
    omega[] = Gamma1 / (pi * sq(a1)) * exp(-(sq(x + D0 / 2) + sq(y)) / sq(a1)) + Gamma2 / (pi * sq(a2)) * exp(-(sq(x - D0 / 2) + sq(y)) / sq(a2));
}
